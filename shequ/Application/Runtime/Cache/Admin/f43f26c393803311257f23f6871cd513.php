...
<div class="page-container" style="min-width: 80%;">
        ...
        <div class="courst-contenr-border">
            ...
            <div class="courst-operate"  >
                <div class="operate-item">
									<span class="input-group-btn">
										<button  id="addAdmin" class="btn btn-success" style="border-radius: 10%;font-size: 12px">+&nbsp;添加</button>
            ...
            <table class="table-position table  table-bordered table-hover" style="width: 100%;margin: 0 auto; ">
                ...
                <tr>
                    ...
                    <th style="text-align: center;">户籍地</th>
                    <th style="text-align: center;">部门/职务</th>
                    <th style="text-align: center;">党员</th>
                </tr>
                <?php if(is_array($adminInfo)): $i = 0; $__LIST__ = $adminInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$adminInfo): $mod = ($i % 2 );++$i;?><tr style="font-size: 12px;color: #8C8C8C;cursor: pointer;" onclick="javascript:location.href='/index.php?m=admin&c=information&a=index&id=<?php echo ($adminInfo["id"]); ?>'">
                    <td><?php echo (getCommunity($adminInfo["community"])); ?></td>
                    <td><?php echo ($adminInfo["cardid"]); ?></td>
                    <td><?php echo ($adminInfo["name"]); ?></td>
                    <td><?php echo ($adminInfo["phone"]); ?></td>
                    <td><?php echo (getSex($adminInfo["sex"])); ?></td>
                    <td><?php echo ($adminInfo["birthland"]); ?></td>
                    <td><?php echo ($adminInfo["position"]); ?></td>
                    <td><?php if($adminInfo["policy"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
    ...
    <script type="text/javascript">
        ...
        $('#addAdmin').on('click',function () {
            window.location.href='/index.php?m=admin&c=add&a=index';
        });
        ...
        $('#searchButton').on('click',function () {
            ...
            if (!re.test(searchContent)) {
                //不是数字那就是名字来查
                window.location.href='/index.php?m=admin&c=search&a=index&name='+searchContent;
            }else{
                //是数字就是用身份证来查的
                window.location.href='/index.php?m=admin&c=search&a=index&cardId='+searchContent;
            }
        })
    </script>
